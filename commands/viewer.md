...
2. Start it in the background: `cd "${CLAUDE_PLUGIN_ROOT}/frontend" && bun run dev`
   (serves on http://localhost:1307).
3. Tell the user the viewer is up at http://localhost:1307 and which project's
   `.docs/` it is showing (the viewer's project selector can switch between
   sibling projects' `.docs/` folders).

If port 1307 is already in use, a viewer is likely already running — just give
the user the URL.
